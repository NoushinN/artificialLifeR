---
title: "Population Dynamics and Evolvability"
output: rmarkdown::html_vignette
bibliography: references.bib
vignette: >
  %\VignetteIndexEntry{Population Dynamics and Evolvability}
...
This article explains population dynamics and evolvability in artificial-life models. Population dynamics describe how population size and composition change over time. Evolvability refers to a system’s capacity to generate heritable variation that can support adaptive change [@kauffman1993origins; @nowak2006evolutionary].
...
The function `simulate_population_dynamics()` provides a simplified way to explore population-level change.
...
If the summary includes trait variation, such as `trait_sd`, it can help show whether the population is becoming more or less diverse.

```{r trait-variation-plot}
...
```{r evolvability-mutation-compare}
...
Evolvability depends on continuity, variation, selection, and constraint. `artificialLifeR` provides simplified models that make these relationships visible and teachable while preserving the distinction between toy simulations and real biological evolution.
