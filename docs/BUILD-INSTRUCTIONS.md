...
- Keep `README.md` as plain markdown. Do not use R Markdown chunks like
  `{r}. Use plain`r code blocks.
- Keep only one `extra_css` field in `_pkgdown.yml`.
- If GitHub blocks a push because of email privacy, set your GitHub
  noreply email with `git config --global user.email` and amend the
  commit author.
